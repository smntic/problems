...
export const contests: Contest[] = [
...

  {
    ...
    problems: [
      ...
      { name: "Utopia Relationships", index: "M", status: ProblemStatus.Solved, notes: "You can create a bipartite graph where the nodes on the left are giving flow and the nodes on the right are receiving flow (left and right node for each node in the original graph). If you push 10k flow into each of the left nodes and find some max flow, then you would expect the max flow to be 10k*n. If you represent the flow between the two layers as a matrix where mat[x][y] represents the amount of flow going from x on the left to y on the right. An important property of square matrices is that A + A^T is symmetric, which is equivalent to the reciprocity requirement. However, this would cause the total flow for each node to be 20k... you can just push 5k into the nodes instead, then the total flow for each node is 10k." },
    ],
  },
...

  {
    ...
    problems: [
      ...
      { name: "It’s About Time", index: "F", status: ProblemStatus.Solved, notes: "Reading comprehension problem. You can just iterate over n1, n2 and n3 and find which best approximates the tropical years. Careful with overflow." },
      ...
      { name: "Simple Solitaire", index: "J", status: ProblemStatus.Solved, notes: "Reading comprehension problem. Just simulate the process per card. Use pairs to handle the precedence, or make a custom struct." },
      { name: "Two Charts Become One", index: "K", status: ProblemStatus.Solved, notes: "You can parse the strings using DFS or a stack and then compare the adjacency lists or recursively compare the trees." },
